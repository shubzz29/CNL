...
#all operations for classful
def classful():
	print("******CLASSFUL********")
	
	try:
		ip_addr=input("Enter IP Address to assign to this machine===")
		ip_net=ipaddress.IPv4Network(ip_addr);				#checking purpose only
		try:
			ip_net=ipaddress.IPv4Address(ip_addr);
		except Exception:
			print("Use classful notation(probably u have put '/' like classless)")
		else:
			try:
				no_of_subnets=int(input("enter No of subnets u want to make=="))
				n=math.ceil(math.log(no_of_subnets,2))

				(mask,ip_id)=findClass(ip_addr,n)
				ip_net_id=ip_id+"/"+mask
